...
function handleRowChange(inputElement) {
    const row = inputElement.parentNode.parentNode.parentNode;

    const min = row.querySelector('#id_min_people');
    const max = row.querySelector('#id_max_people');
    const price = row.querySelector('#id_price');
    const currency = row.querySelector('#id_currency');

    if (min.value !== '' || max.value !== '' || price.value !== '' || currency.value !== ''){
